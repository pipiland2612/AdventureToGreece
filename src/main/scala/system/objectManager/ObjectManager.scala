package system.objectManager

import entities.`object`.*
import entities.Entity
import entities.creatures.enemy.*
import entities.creatures.{Captain, Merchant, Socerer}
import entities.items.InteractiveObjects
...
class ObjectManager(var gp : GamePanel) :
  ...
  def setEnemy (): Unit =
    // OBJ                                      INDEX  POSITION    (COL, ROW)
    val enemyData0 = List(
      (new Beetle(gp),                       0,     Some(calcPos (12, 10))),
      (new Mantis(gp),                       1,     Some(calcPos (6, 8))),
      (new Beetle(gp),                       3,     Some(calcPos (20, 16))),
      (new Beetle(gp),                       8,     Some(calcPos (42, 14))),
      (new Beetle(gp),                       9,     Some(calcPos (39, 6))),
      (new Beetle(gp),                       10,    Some(calcPos (36, 3))),
      (new Orc_Light(gp),                    11,    Some(calcPos (5, 1))),
    )

    this.loadEntity(enemyData0, gp.enemyList(0))

    val enemyData1 = List(
      (new Necromancer(gp),                  0,     Some(calcPos (23, 24))),
      (new Necromancer(gp),                  1,     Some(calcPos (40, 2))),

      (new Necromancer(gp),                  2,     Some(calcPos (13, 12))),
      (new Orc_Heavy(gp),                    3,     Some(calcPos (5, 10))),
      (new Orc_Heavy(gp),                    4,     Some(calcPos (35, 4))),
      (new Necromancer(gp),                  5,     Some(calcPos (9, 19))),
      (new Orc_Heavy(gp),                    6,     Some(calcPos (37, 13))),
      (new Orc_Heavy(gp),                    7,     Some(calcPos (35, 45))),
    )
    this.loadEntity(enemyData1, gp.enemyList(1))

    val enemyData2 = List(
        (new KingOfDeath(gp),                  0,     Some(calcPos (23, 18))),
    )
